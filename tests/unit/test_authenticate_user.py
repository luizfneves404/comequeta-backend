...
from tests.fakes import FakePasswordHasher, FakeTokenProvider, FakeUserRepository


def make_context() -> AuthenticateUser:
    users = FakeUserRepository()
    hasher = FakePasswordHasher()
    RegisterUser(users, hasher).execute(email="ana@example.com", name="Ana", password="secret123")
...
def test_rejects_wrong_password() -> None:
    ...
    with pytest.raises(InvalidCredentials):
        authenticate.execute(email="ana@example.com", password="wrong-password")
